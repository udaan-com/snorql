    ...
    override fun getMetricRecommendations(
        ...
        if(metricInput.recommendationRequired) {
            val indexListToBeDropped: List<String> = metricResult.queryList
                ...
                .map { it.indexName }
            return if (indexListToBeDropped.isEmpty()) TableUnusedIndexRecommendation("No action required.")
            else TableUnusedIndexRecommendation("Drop indexes: ${indexListToBeDropped.joinToString(",")} to save storage.")
        }
        return null
    }
